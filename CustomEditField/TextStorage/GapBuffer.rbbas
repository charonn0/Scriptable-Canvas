...
Class GapBuffer
	...
	#tag Method, Flags = &h0
		Function getByteAt(offset as Integer) As Integer
		  //if offset before gap
		  if offset < gapStart then
		    Return buffer.ByteAt(offset)
		  end if
		  
		  //else, offset it by length
		  Return buffer.ByteAt(offset + GapLength)
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub replace(index as integer, length as integer, text as string)
		  ...
		  #if DebugBuild
		    lastOpMicros = Microseconds - lastOpMicros
		  #endif
		End Sub
	#tag EndMethod
	...
	#tag Property, Flags = &h1
		Protected lastOpMicros As double
	#tag EndProperty
	...
	#tag ComputedProperty, Flags = &h0
		#tag Getter
			Get
			  Return "buffer physical: "+str(buffer.Size)+", logical: "+str(Length)+", gap: "+str(GapLength)+", lastOp µs: "+str(lastOpMicros)
			End Get
		#tag EndGetter
		Stats As string
	#tag EndComputedProperty
